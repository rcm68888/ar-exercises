...
@mens_stores = Store.where(mens_apparel: true).order(annual_revenue: :desc)
@mens_stores.each do |mstore| 
  puts "#{mstore.name} has annual revenue of #{mstore.annual_revenue}."
end

@womens_stores = Store.where(["womens_apparel = ? and annual_revenue < ?", "true", "1000000"])

@womens_stores.each do |wstore|
  puts "#{wstore.name} has annual revenue of #{wstore.annual_revenue}."
end